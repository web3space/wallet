            ...
            .pug#hbreadcrumb.pull-right.m-t-lg
              ol.pug.hbreadcrumb.breadcrumb
                ...
                li.pug
                  span.pug App views
                li.pug.active
                  span.pug Code editor
            h2.pug.font-light.m-b-xs
              span.pug Code editor
            small.pug Versatile text editor implemented in JavaScript for the browser.
      .pug.content
        .pug.row
          .pug.col-lg-12
            .pug.hpanel
              ...
              .pug.panel-body
                p.pug
                  span.pug CodeMirror is a versatile text editor implemented in JavaScript for the browser. It is specialized for editing code, and comes with a number of language modes and addons that implement more advanced editing functionality.
                textarea.pug#code1.form-control
                  span.pug // EDITOR CONSTRUCTOR
                  span.pug // A CodeMirror instance represents an editor. This is the object
                  span.pug // that user code is usually dealing with.
                  span.pug function CodeMirror(place, options) {
                  span.pug if (!(this instanceof CodeMirror)) return new CodeMirror(place, options);
                  span.pug this.options = options = options ? copyObj(options) : {};
                  span.pug // Determine effective options based on given values and defaults.
                  span.pug copyObj(defaults, options, false);
                  span.pug setGuttersForLineNumbers(options);
                  span.pug var doc = options.value;
                  span.pug if (typeof doc == &quot;string&quot;) doc = new Doc(doc, options.mode);
                  span.pug this.doc = doc;
                  span.pug var display = this.display = new Display(place, doc);
                  span.pug display.wrapper.CodeMirror = this;
                  span.pug updateGutters(this);
                  span.pug themeChanged(this);
                  span.pug if (options.lineWrapping)
                  span.pug this.display.wrapper.className += &quot; CodeMirror-wrap&quot;;
                  span.pug if (options.autofocus &amp;&amp; !mobile) focusInput(this);
                  span.pug this.state = {
                  span.pug keyMaps: [],  // stores maps added by addKeyMap
                  span.pug overlays: [], // highlighting overlays, as added by addOverlay
                  span.pug modeGen: 0,   // bumped when mode/overlay changes, used to invalidate highlighting info
                  span.pug overwrite: false, focused: false,
                  span.pug suppressEdits: false, // used to disable editing during key handlers when in readOnly mode
                  span.pug pasteIncoming: false, cutIncoming: false, // help recognize paste/cut edits in readInput
                  span.pug draggingText: false,
                  span.pug highlight: new Delayed() // stores highlight worker timeout
                  span.pug };
                  span.pug // Override magic textarea content restore that IE sometimes does
                  span.pug // on our hidden textarea on reload
                  span.pug if (ie &amp;&amp; ie_version &lt; 11) setTimeout(bind(resetInput, this, true), 20);
                  span.pug registerEventHandlers(this);
                  span.pug ensureGlobalHandlers();
                  span.pug startOperation(this);
                  span.pug this.curOp.forceUpdate = true;
                  span.pug attachDoc(this, doc);
                  span.pug if ((options.autofocus &amp;&amp; !mobile) || activeElt() == display.input)
                  span.pug setTimeout(bind(onFocus, this), 20);
                  span.pug else
                  span.pug onBlur(this);
                  span.pug for (var opt in optionHandlers) if (optionHandlers.hasOwnProperty(opt))
                  span.pug optionHandlers[opt](this, options[opt], Init);
                  span.pug maybeUpdateLineNumberWidth(this);
                  span.pug for (var i = 0; i &lt; initHooks.length; ++i) initHooks[i](this);
                  span.pug endOperation(this);
                  span.pug }
