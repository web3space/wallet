        ...
        .pug.hpanel
          .pug.panel-body
            ...
            h2.pug.font-light.m-b-xs
              span.pug Package
            small.pug HOMER v.1.9 package contents
      .pug.content
        .pug.row
          .pug.col-lg-6
            .pug.hpanel.hgreen
              .pug.panel-body
                h3.pug HTML/jQuery version
                .pug.text-muted.font-bold.m-b-xs
                  ul.pug
                    li.pug Full version
                    li.pug Seed Project
                p.pug
                  span.pug This version contains all components and plugin created with strict HTML and jQuery. This version is great if you are a PHP, Ruby or JAVA programmer. As each page is created separately it allows you to quickly implement it for your project by including needed resources. The code is very clean and clear so you should not have any problems with its implementation.
          .pug.col-lg-6
            .pug.hpanel.hyellow
              .pug.panel-body
                h3.pug ASP.NET MVC version
                .pug.text-muted.font-bold.m-b-xs
                  ul.pug
                    li.pug MVC5 Full version
                    li.pug MVC5 Seed Project
                    li.pug MVC6 Full version
                    li.pug MVC6 Seed Project
                p.pug
                  span.pug ASP.NET MVC is supported ASP.NET version of HOMER theme. This version contains a ASP.NET MVC5 and ASP.NET Core 1.0 project written in Visual Studio for .net developers. Project has implemented all the plug-ins and it is created in accordance with the convention MVC - Razor engine.
          .pug.col-lg-6
            .pug.hpanel.hviolet
              .pug.panel-body
                h3.pug AngularJS version
                .pug.text-muted.font-bold.m-b-xs
                  ul.pug
                    li.pug Full version
                    li.pug Seed Project
                p.pug
                  span.pug Angular version is built with the latest technology contains well organized and coded controllers and directives written specifically to support angular framework (AngularJS 1.6.3) . Angular version is written with Opinionated AngularJS styleguide for teams. It is provided with full Grunt/Bower environment. In project folder you find also SeedProject. It is an application skeleton for a typical AngularJS web app.
          .pug.col-lg-6
            .pug.hpanel.hblue
              .pug.panel-body
                h3.pug METEOR version
                .pug.text-muted.font-bold.m-b-xs
                  ul.pug
                    li.pug Full version
                    li.pug Seed Project
                p.pug
                  span.pug This version contains Meteor project (METEOR@1.4.3.2) with all designs and components from Homer theme. Project is created in accordance with the convention of Meteor structure and Blaze templating. (Great for for discover Meteor framework.)
        .pug.row
          .pug.col-lg-12
            .pug.hpanel.hred
              .pug.panel-body
                h3.pug LESS + SCSS + PSD
                .pug.text-muted.font-bold.m-b-xs
                  ul.pug
                    li.pug LESS files (well organized by components, pages, plugins, and utilities)
                    li.pug SCSS files
                    li.pug PSD files - 37 files full layered
                p.pug
                  span.pug Project contains also css preprocessors files for LESS and SCSS. Additional we prepared 37 full layered .psd files for designer.
