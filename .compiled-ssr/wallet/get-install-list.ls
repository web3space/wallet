...
save-registry = (registry)->
    ...
    local-storage.set-item name, resulted-string
...
ask-user = (cweb3, store, plugin, cb)->
    ...
    return ask store, "Would you like to install #{plugin.type} `#{plugin.token}` ?", cb if registry.index-of(plugin.token) is -1
