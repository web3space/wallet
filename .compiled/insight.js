...
(function(){
  var moment, whitebox, ref$, map, foldl, any, each, find, sum, filter, get, post, plus, minus, div, times, BitcoinLib, jsonParse, getFullpairByIndex, deadline, calcFee, getKeys, extend, getDec, addValue, getOutputs, createTransaction, pushTx, getBalance, transformTx, getTransactions, out$ = typeof exports != 'undefined' && exports || this, toString$ = {}.toString;
  ...
  out$.createTransaction = createTransaction = function(arg$, cb){
    ...
    return getOutputs({
      ...
      sign = function(output, i){
        var key;
        key = BitcoinLib.ECPair.fromWIF(sender.privateKey, network);
  ...
  out$.getBalance = getBalance = function(arg$, cb){
    ...
    return get(network.api.url + "/api/addr/" + address + "/balance").timeout({
      deadline: deadline
