...
(function(){
  var stringify, ref$, filter, map, foldl, each, find, sum, plus, minus, times, div, get, post, jsonParse, getFullpairByIndex, BitcoinLib, deadline, calcFee, getKeys, extendNum, extend, toHex, getApiUrl, addValue, getOutputs, createTransaction, transformTx, getTransactions, getDec, checkDecodedData, pushTx, checkTxStatus, str, getBalance, out$ = typeof exports != 'undefined' && exports || this, toString$ = {}.toString;
  stringify = require('qs').stringify;
  ref$ = require('prelude-ls'), filter = ref$.filter, map = ref$.map, foldl = ref$.foldl, each = ref$.each, find = ref$.find, sum = ref$.sum;
  ...
  out$.getBalance = getBalance = function(arg$, cb){
    ...
    return post(apiUrl + "/v1/address/addr/", req).type('form').end(function(err, data){
      ...
      if (toString$.call(data.body.balance).slice(8, -1) !== 'Array') {
      ...
      data.body.balance);
      if (balance == null) {
        return cb("Cannot obtain the balance");
      }
      dec = getDec(network);
